...
function UpdateBirthday({ value }: { value: string }) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/user/update", { key: "birthday", value: input }, { withCredentials: true })
  ...
  return (
    <div className='space-y-2 border-b-1 border-b-black flex items-center justify-between'>
      <label className='font-semibold text-md' htmlFor="name">Birthday</label>
      ...
      {edit && <div className='flex gap-1'>
        <input type="text" placeholder='eg. 12 May 2000' className='p-1 rounded-lg' name="dob" onChange={(e) => setInput(e.target.value)} value={input} />
