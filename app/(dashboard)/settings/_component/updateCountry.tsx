...
function UpdateCountry({ value }: { value: string }) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/user/update", { key: "country", value: input }, { withCredentials: true })
  ...
  return (
    <div className='space-y-2 border-b-1 border-b-black flex items-center justify-between'>
      <label className='font-semibold text-md' htmlFor="name">Country</label>
      {!edit && <><p className=''>{defaultValue}</p><Button onClick={() => setEdit(prev => !prev)} variant={"link"}>Edit</Button></>}
      {edit && <div className='flex gap-1'>
        <input type="text" placeholder='Enter new Country' className='p-1 rounded-lg' name="country" onChange={(e) => setInput(e.target.value)} value={input} />
