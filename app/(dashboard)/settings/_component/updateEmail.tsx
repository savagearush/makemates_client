...
function UpdateEmail({ value }: { value: string }) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/user/update", { key: "email", value: input }, { withCredentials: true })
  ...
  return (
    <div className='space-y-2 border-b-1 border-b-black flex items-center justify-between'>
      ...
      {edit && <div className='flex gap-1'>
        <input type="text" placeholder='Enter new email' className='p-1 rounded-lg' name="email" onChange={(e) => setInput(e.target.value)} value={input} />
